...
|---|---|---|
| Interface | Streamlit console | Streamlit console **plus** a Next.js portal (login, farmer, bank, climate, marketplace, export, admin pages) |
| Backend | Logic inside the Streamlit app | Separate **FastAPI service** with JWT sessions and optional API keys for bank and integrator access |
| User roles | 4 (Cooperative, Bank/SACCO Officer, Researcher, Admin) | 5, adding **Trader / Buyer** |
| New capabilities | | Farmer and trader **marketplace**, **seasonal climate outlook**, **export crop advisory** |
| Password storage | SHA-256 with one shared salt | PBKDF2-HMAC-SHA256, 200,000 iterations, random salt per user |
| County list | 26 selectable in the interface | Driven by the data: all 35 counties |
| Storage | Local files | Local SQLite/JSON by default, optional Supabase |
| Deployment | Docker | Docker Compose (backend and frontend) |
| Automated tests | 27 | 95 test functions |


## 👥 13. Authors & Capstone Team
 - *Carson Sila, Charlene Kamunyu, , Emmanuel Brian, Brian Mugambi (MVP2 contributor, also contributed to doinggbits_ingestion branch)*
